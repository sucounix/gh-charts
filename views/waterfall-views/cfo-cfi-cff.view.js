...
const renderCFOView = (waterfallRequest) => {
  ...
  */
  (() => {
    ...
    for (const row of waterfallData.rows.slice(0, 3)) {
      ...
      svgHTML += `
        <line
             ...
             <text
                  ...
                  y="20"
                  text-anchor="middle"
                  style="font-size: 16px; font-weight: 500; font-family: ${fontFamily};"
                >
                  ${
                    graphIntervals[i] >= 0
                      ? graphIntervals[i] === 0
    ...
    if (preparedData) {
      ...
      if (tables.length > 0) {
        for (let i = 0; i < tables.length; i++) {
          actualHeight +=
            YSegmentHeight * tables[i].children.length +
            1.2 * tableBufferHeight;
        }
      }
        ...
        */
        svgHTML += `
           <text
            ...
            fill="black"
            style="font-size: 14px; font-weight: 400; line-height: 24px; letter-spacing: -0.84px;"
          >
            ${entry.name}
          </text>
        `;
    ...
    shownTables.forEach((table) => {
      if (table.name !== "Totals") {
        ...
        svgHTML += `
         <line
          x1="0"
          y1=${table.startY + YSegmentHeight * table.children.length * 1.01}
          x2="100%"
          y2=${table.startY + YSegmentHeight * table.children.length * 1.01}
          stroke="white"
          stroke-width="15"
        />
        <line
      ...
      if (table.name === "Totals") {
        ...
        <line
          x1="0"
          y1=${table.startY + table.children.length * YSegmentHeight - 45}
          x2="100%"
          y2=${table.startY + table.children.length * YSegmentHeight - 45}
          stroke="black"
          stroke-width="1"
        />
      `;
