...
const getBreakeven = async (req, res) => {
  ...
  const YValueToPixels = (value) => {
    ...
    const minValue = 0;
    const pixelsPerUnit = svgHeight / (maxValue - minValue);
    const y = svgHeight - (value - minValue) * pixelsPerUnit;
    return y;
  };
  ...
  */
  (() => {
    for (let i = 0; i < 11; i++) {
      ...
      y2="0"
      stroke="#000"
      transform="translate(0,${(svgHeight / 10) * i})"
      stroke-dasharray="12"
      stroke-opacity="0.2"
    />`;
    }
  ...
  */
  (() => {
    for (let i = 0; i < 21; i++) {
      gridString += `<line
        x1="${labelsWidth + ((svgWidth - labelsWidth) / 20) * i}"
        x2="${labelsWidth + ((svgWidth - labelsWidth) / 20) * i}"
        y1="0"
        y2="100%"
        stroke="#000"
        stroke-opacity="0.1"
  ...
  */
  (() => {
    ...
    gridString += `<text
      ...
      font-size="12"
    >
      ${handleCurrencySymbol(currency)} 0
    </text>`;
  })();
  ...
  (() => {
    for (let i = 0; i < 21; i++) {
      ...
      <circle
        ...
        cy=${YValueToPixels((topRevenue / 20) * i)}
        r="4"
        fill="#2F9E44"
      ...
      />
        ...
        r="4"
        fill="#086972"
        transform="translate(${labelsWidth}, 0)"
      />
      <circle
      cx=${((svgWidth - labelsWidth) / 20) * i}
      cy=${YValueToPixels(
  ...
  */
  (() => {
    /* 
      Cost Line Point
    */
    const costLineX = labelsWidth + (svgWidth - labelsWidth) / 2;
    const costLineY =
      breakeven_data.fixed_cost +
    ...
    /* 
      Revenue Line Point
    */
    const revenueLineX = labelsWidth + (svgWidth - labelsWidth) / 2;
